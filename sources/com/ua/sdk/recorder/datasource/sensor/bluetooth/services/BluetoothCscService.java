...
public class BluetoothCscService implements BaseGattCallback.GattCallbackListener {
    ...
    public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        ...
        if (status == 0 && characteristic.getUuid().equals(UUID.fromString(GattAttributes.RSC_MEASUREMENT_CHARACTERISTIC)) && (characteristic.getIntValue(17, 0).intValue() & 1) != 0 && (controlPoint = gatt.getService(UUID.fromString(GattAttributes.CSC_SERVICE)).getCharacteristic(UUID.fromString(GattAttributes.CONTROL_POINT_CHARACTERISTIC))) != null) {
    ...
    private void parseCharacteristic(BluetoothGattCharacteristic characteristic) {
        if (UUID.fromString(GattAttributes.CSC_MEASUREMENT_CHARACTERISTIC).equals(characteristic.getUuid())) {
            ...
            long wheelRevs = -1;
            if ((flags & 1) != 0) {
                wheelRevs = (long) characteristic.getIntValue(20, offset).intValue();
                offset += 4;
            }
            long crankRevs = -1;
            if ((flags & 2) != 0) {
                crankRevs = (long) characteristic.getIntValue(18, offset).intValue();
            }
