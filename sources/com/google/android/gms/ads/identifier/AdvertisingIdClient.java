...
public class AdvertisingIdClient {
    ...
    public Uri zza(Info info, boolean z, Throwable th) {
        ...
        bundle.putString("app_context", z ? ActivityType.GENERIC : "0");
        if (info != null) {
            bundle.putString("limit_ad_tracking", info.isLimitAdTrackingEnabled() ? ActivityType.GENERIC : "0");
        }
        if (!(info == null || info.getId() == null)) {
            bundle.putString("ad_id_size", Integer.toString(info.getId().length()));
        }
        if (th != null) {
            bundle.putString(Constants.Params.ERROR, th.getClass().getName());
        }
