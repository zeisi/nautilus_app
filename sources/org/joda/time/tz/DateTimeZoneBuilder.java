    ...
    private static final class PrecalculatedZone extends DateTimeZone {
        ...
        static PrecalculatedZone create(String str, boolean z, ArrayList<Transition> arrayList, DSTZone dSTZone) {
            ...
            while (i3 < strArr2.length - 1) {
                ...
                Period period = new Period(jArr[i3], jArr[i3 + 1], PeriodType.yearMonthDay(), (Chronology) instanceUTC);
                if (j != j2 && j3 == j4 && str3.equals(str4) && period.getYears() == 0 && period.getMonths() > 4 && period.getMonths() < 8 && str3.equals(strArr[2]) && str3.equals(strArr[4])) {
                    if (ZoneInfoLogger.verbose()) {
                        ...
                        System.out.println("     - " + new DateTime(jArr[i3], (Chronology) instanceUTC) + Constants.HYPHEN + new DateTime(jArr[i3 + 1], (Chronology) instanceUTC));
                    }
                    if (j > j2) {
                        strArr2[i3] = (str3 + "-Summer").intern();
                    } else if (j < j2) {
                        strArr2[i3 + 1] = (str4 + "-Summer").intern();
                        i3++;
                    }
                }
                i3++;
